...
export function SuitesGrid() {
  return (
    <section className="bg-hotel-cream py-20 md:py-28" id="suites">
...
function RoomCard({ suite }: { suite: (typeof suites)[number] }) {
  ...
  const prevImage = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setCurrentIndex((prev) => (prev === 0 ? suite.images.length - 1 : prev - 1));
  };

  const nextImage = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setCurrentIndex((prev) => (prev === suite.images.length - 1 ? 0 : prev + 1));
  };
